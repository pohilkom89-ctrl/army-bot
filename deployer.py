...
COPY marketplace_main.py /app/marketplace_main.py
...
def _ensure_runtime_files(bot_dir: Path) -> None:
    ...
    for name in ("usage_reporter.py", "vk_main.py", "max_main.py", "lms_main.py", "marketplace_main.py"):
        src = RUNTIME_DIR / name
        if not src.exists():
            raise FileNotFoundError(
                f"deployer: runtime helper {src} missing — cannot build bot images"
            )
        shutil.copy2(src, bot_dir / name)
...
def write_bot_rate_limit(bot_id: int, max_per_hour: int) -> None:
    """Write rate_limit.txt for the bot. 0 means disabled."""
    bot_dir = _bot_dir(bot_id)
    bot_dir.mkdir(parents=True, exist_ok=True)
    (bot_dir / "rate_limit.txt").write_text(str(max(0, max_per_hour)), encoding="utf-8")
...
def prepare_max_bot_files(bot_id: int, system_prompt: str) -> Path:
    """Prepare build context for a MAX bot. Uses max_main.py as entrypoint."""
    bot_dir = _bot_dir(bot_id)
    bot_dir.mkdir(parents=True, exist_ok=True)
    _write_system_prompt(bot_dir, system_prompt)
    ...
    if not (bot_dir / "main.py").exists():
        (bot_dir / "main.py").write_text("# max bot placeholder\n", encoding="utf-8")
    _write_dockerfile(bot_id)
    _ensure_runtime_files(bot_dir)
    logger.info("deployer: prepared MAX files for bot_id={}", bot_id)
    return bot_dir


async def deploy_max_bot(bot_id: int) -> str:
    """Deploy a MAX messenger bot. Uses max_main.py entrypoint."""
    bot = await get_bot_by_id_any(bot_id)
    if bot is None:
        raise RuntimeError(f"deployer: no BotConfig for bot_id={bot_id}")
    max_token = bot.bot_token
    if not max_token:
        raise RuntimeError(f"deployer: bot_token (MAX_TOKEN) empty for bot_id={bot_id}")
...
def _deploy_max_sync(
    ...
    if docker.container.exists(name):
        state = _container_state(name)
        if state == "running":
            logger.info("deployer: MAX {} already running — noop", name)
            return _container_id(name)
        if state in ("exited", "created"):
            docker.container.start(name)
            return _container_id(name)
    ...
    build_bot_image(bot_dir, bot_id)
    tag = _image_tag(bot_id)
    logger.info("deployer: starting MAX container {}", name)
    container = docker.run(
        image=tag,
        name=name,
        detach=True,
...
def prepare_marketplace_bot_files(bot_id: int, system_prompt: str) -> Path:
    """Prepare build context for a marketplace seller assistant bot."""
    bot_dir = _bot_dir(bot_id)
    bot_dir.mkdir(parents=True, exist_ok=True)
    _write_system_prompt(bot_dir, system_prompt)
    for fname, default in [
        ...
        if not (bot_dir / fname).exists():
            (bot_dir / fname).write_text(default, encoding="utf-8")
    if not (bot_dir / "main.py").exists():
        (bot_dir / "main.py").write_text("# marketplace bot placeholder\n", encoding="utf-8")
    _write_dockerfile(bot_id)
    _ensure_runtime_files(bot_dir)
    logger.info("deployer: prepared marketplace files for bot_id={}", bot_id)
    return bot_dir


async def deploy_marketplace_bot(bot_id: int) -> str:
    """Deploy a marketplace seller assistant bot."""
    bot = await get_bot_by_id_any(bot_id)
    if bot is None:
        raise RuntimeError(f"deployer: no BotConfig for bot_id={bot_id}")
    bot_token = bot.bot_token
    if not bot_token:
        raise RuntimeError(f"deployer: bot_token empty for marketplace bot_id={bot_id}")
    ...
    return await asyncio.to_thread(
        _deploy_marketplace_sync,
        bot_id, bot_token, openrouter_key, model_bots, factory_url,
        internal_api_key, system_prompt,
    )


def _deploy_marketplace_sync(
    bot_id: int,
    bot_token: str,
    openrouter_key: str,
    model_bots: str,
    ...
    if docker.container.exists(name):
        state = _container_state(name)
        if state == "running":
            logger.info("deployer: marketplace {} already running — noop", name)
            return _container_id(name)
        if state in ("exited", "created"):
            docker.container.start(name)
            return _container_id(name)
    ...
    build_bot_image(bot_dir, bot_id)
    tag = _image_tag(bot_id)
    logger.info("deployer: starting marketplace container {}", name)
    container = docker.run(
        image=tag,
        name=name,
        detach=True,
        ...
        memory=CONTAINER_MEMORY_LIMIT,
        add_hosts=[("host.docker.internal", "host-gateway")],
        command=["python", "marketplace_main.py"],
        envs={
            "BOT_TOKEN": bot_token,
            "BOT_ID": str(bot_id),
            "OPENROUTER_API_KEY": openrouter_key,
    ...
    container_id = getattr(container, "id", str(container))
    logger.info("deployer: marketplace container {} started (id={})", name, container_id)
    return container_id
