...
async def log_message_endpoint(request: web.Request) -> web.Response:
    ...
    provided = request.headers.get("X-Internal-Key", "")
    if not secrets.compare_digest(provided, expected):
        logger.warning("log_message: bad key from {}", request.remote)
        return web.Response(status=401, text="unauthorized")
    try:
        data = await request.json()
        bot_id = int(data["bot_id"])
    ...
    if role not in ("user", "bot"):
        return web.Response(status=400, text="role must be 'user' or 'bot'")
    try:
        ...
        logger.exception("log_message: failed bot_id={}", bot_id)
        return web.Response(status=500, text="error")
    return web.Response(status=200, text="ok")
